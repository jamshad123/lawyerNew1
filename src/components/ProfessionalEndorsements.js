                    ...
                    return (
                        <div key={item.id} className='mb-4  px-4 py-4 bg-white md:rounded-lg rounded-md flex items-center justify-between'>
                            <div>
                                ...
                                <p className='text-text-secondary text-[0.8rem] sm:text-[0.9rem] md:text-[1rem]' style={{fontFamily:"Open Sans", fontStyle:"regular",fontWeight:"700"}}>{item.endorsements} Endorsements | {item.recommendations} Recommendations</p>
                            </div>
                            <span className='p-2 bg-yellow rounded-full ml-[10px]' style={{backgroundColor:"#ffcc00"}}><SlFrame /></span>
                        </div>
                    )
                })
            }
